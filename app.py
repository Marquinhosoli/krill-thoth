...
MODEL_FRUTAS = BASE_DIR / "models" / "modelo_frutas.xlsx"
MODEL_LEGUMES = BASE_DIR / "models" / "modelo_legumes.xlsx"
...
def read_order(file) -> pd.DataFrame:
    ...
    df = df[["Loja", "Descrição do Produto", "Qtde."]].copy()
...
def model_map(ws):
    ...
    for col in range(2, ws.max_column + 1):
        top = norm_key(ws.cell(1, col).value)
        line2 = norm_text(ws.cell(2, col).value)
        if "TOTAL" in top or line2.upper() == "TOTAL":
            total_col = col
            continue
        if "CD" in top:
            cd_col = col
            continue
        if re.fullmatch(r"\d+", line2):
            store_to_col[line2] = col
    if not store_to_col:
        raise ValueError("Não consegui mapear as lojas pela linha 2 do modelo.")
...
def build_unknown(unknown: pd.DataFrame) -> bytes:
    out = BytesIO()
    df = pd.DataFrame({"PRODUTO": sorted(unknown.index.tolist())}) if not unknown.empty else pd.DataFrame(columns=["PRODUTO"])
    with pd.ExcelWriter(out, engine="openpyxl") as writer:
...
with st.sidebar:
    ...
    st.info("O sistema usa a linha 2 do modelo para mapear as lojas.")
...
if st.button("PROCESSAR", use_container_width=True, type="primary"):
    if not uploaded:
        ...
        try:
            if not MODEL_FRUTAS.exists() or not MODEL_LEGUMES.exists():
                raise FileNotFoundError("Os modelos fixos não foram encontrados dentro do app.")
